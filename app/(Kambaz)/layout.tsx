...
import Breadcrumb from "./Courses/[cid]/Breadcrumb"; // breadcrumb component
import { courses } from "./Database";       // your courses data
...
export default function KambazLayout({ children }: Readonly<{ children: ReactNode }>) {
  ...
  // Get course object for breadcrumb
          ...
          <ListGroup className="rounded-0 position-fixed bottom-0 top-0 d-none d-sm-block bg-black z-2" style={{ width: 110 }}
            ...
            <ListGroupItem className={`border-0 ${isAccountActive ? 'bg-white' : 'bg-black'} text-center`}>
              <Link href="/Account" id="wd-account-link"
                    className={`${isAccountActive ? 'text-danger' : 'text-white'} text-decoration-none`}>
            ...
            <ListGroupItem className={`border-0 ${isDashboardActive ? 'bg-white' : 'bg-black'} text-center`}>
              <Link href="/Dashboard" id="wd-dashboard-link"
                    className={`${isDashboardActive ? 'text-danger' : 'text-white'} text-decoration-none`}>
            ...
            <ListGroupItem className={`border-0 ${isCoursesActive ? 'bg-white' : 'bg-black'} text-center`}>
              <Link href="/Courses/1234" id="wd-course-link"
                    className={`${isCoursesActive ? 'text-danger' : 'text-white'} text-decoration-none`}>
            ...
            <ListGroupItem className="border-0 bg-black text-center">
              <a href="https://calendar.google.com/calendar" id="wd-calendar-link" target="_blank"
            ...
            <ListGroupItem className="border-0 bg-black text-center">
              <a href="https://outlook.office.com/mail/inbox/" id="wd-inbox-link" target="_blank"
                 className="text-white text-decoration-none">
            ...
            <ListGroupItem className={`border-0 ${isLabsActive ? 'bg-white' : 'bg-black'} text-center`}>
              <Link href="/Labs" id="wd-labs-link"
                    className={`${isLabsActive ? 'text-danger' : 'text-white'} text-decoration-none`}>
